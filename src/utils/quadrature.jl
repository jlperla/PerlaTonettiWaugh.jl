...
function ω_weights(z, α, ξ)
    ...
    interiorWeights = [f_vec[i]/2 * (Δ[i] + Δ[i+1]) for i = 2:M-1] # Turn these into interior trapezoidal weights. (19)
    return [f_vec[1]/2 * Δ[2]; interiorWeights; f_vec[M]/2 * Δ[M]] # Add the boundary weights. (19, with ghost node definitions)
end
