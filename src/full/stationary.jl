...
function stationary_numerical(params, z, init_x = defaultiv(params); kwargs...)
    ...
    function steadystatewelfare(γ)
        c = (1 - L_tilde)*z_bar - η*ζ*Ω_T*Theta*(S + δ/χ) # (eq:H.22)
        if γ == 1
            U = ρ*log(c) + g_T # (eq:H.20)
        else
            U = 1/(1 - γ) * (c^(1-γ))/(ρ+(γ-1)*g_T) # (eq:H.20)
        end
        return U
    end

    return merge(staticvalues, (g = g_T, z_hat = z_hat_T, Ω = Ω_T, v_tilde = v_tilde, U = steadystatewelfare(γ)))
end
