...
function minimize_residuals_python(params, settings)
    ...
    g_initial = fill(g_T, g_node_count - 1)
    # solve the optimization problem
    sol = DFOGN.solve(x -> calculate_residuals_by_candidate(x, params, settings), g_initial, lower = fill(0., size(g_initial)))
    settings = merge(settings, (g = interpolate_g(DFOGN.optimizer(sol), settings), )) # only runs if converged(sol), otherwise throws error(sol.msg)
    return calculate_residuals(params, settings) # return the resultiing residuals
end
