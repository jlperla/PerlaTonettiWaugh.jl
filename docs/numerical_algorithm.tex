...
Define an irregular extended grid $z^{\text{ex}} = \set{z_i}_{i=0}^{P+1}$ with boundary nodes $z_0 = 0$ and $z_{P+1} = \bar{z}$ is a ``large'' number (keeping in mind that the effective number is $e^{\bar{z}}$), and the corresponding interior grid $z = set{z_i}_{i=1}^P$ that excludes boundary points.
...
\begin{itemize}
	\item Define an irregular extended grid $z^{\text{ex}} = \set{z_i}_{i=0}^{P+1}$ with boundary nodes $z_0 = 0$ and $z_{P+1} = \bar{z}$ is a ``large'' number (keeping in mind that the effective number is $e^{\bar{z}}$), and the corresponding interior grid $z = set{z_i}_{i=1}^P$ that excludes boundary points.
	...
	v^{\text{ex}}(t) &\equiv \set{v(z_i, t)}_{i=0}^{P+1}\in\R^{P+2}\\
...
\intertext{This can be discretized as}
