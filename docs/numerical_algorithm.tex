...
To calculate these, use $\Delta_{-}$ and $\Delta_{+}$ (from \cref{eq:Delta-minus,eq:Delta-plus}), backwards and forward diffs (respectively) for the $z$ grid, padded with (with $\Delta_{0,-} = 0$ and $\Delta_{P+1,+} = 0$):
\begin{align}
	\overline{\Delta}_{-} &\equiv \begin{bmatrix} 0 \\
	\text{diff}(z)
	\end{bmatrix}\in\R^{P+2}\label{eq:Delta-minus}\\
	\overline{\Delta}_{+} &\equiv \begin{bmatrix} \text{diff}(z)\\
	0
\end{bmatrix}\in\R^{P+2}
\intertext{Then define}
\bar{\omega}&\equiv \frac1{2}(\overline{\Delta}_{-} + \overline{\Delta}_{+})\in \R^{P+2}\\
