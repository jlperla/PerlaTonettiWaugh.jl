...
\begin{itemize}
	...
	\item The interest rate $r(t)$ is no longer given exogenously, but needs to be found in equilibrium.  However, since it is forward looking and can be solved in the current structure with only minor changes since the transition dynamics are being solved backwards
	\item The number of varieties $\Omega(t)$ is endogenously determined based on the endogneous free-entry decision, and may change slowly according to profit maximizing and income smoothing incentives
