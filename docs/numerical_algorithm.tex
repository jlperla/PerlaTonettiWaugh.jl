...
To calculate these, use $\Delta_{-}$ and $\Delta_{+}$ backwards and forward diffs (respectively) for the $z$ grid (with $\Delta_{1,-} = 0$ and $\Delta_{P,+} = 0$),
\begin{align}
...
Let $v_1(t)$ be the $1$st element in the $v(t)$ vector, corresponding to $v(\Delta_{0,+},t)$.  From \cref{sec:boundary-extrapolation}, we know that $v(0, t) = \Xi_1 v(\Delta_{0,+},t)$  Then value matching condition in \cref{eq:simple-summary2-numerical,eq:simple-summary1-numerical} along with the system of ODEs becomes the system of differential-algebraic equations (DAE)
\begin{align}
