...
In order to discretize \cref{eq:simple-summary2-numerical}, we need to calculate the integral using quadrature. 

Given the irregular grid, use the weights for the non-uniform trapezoidal rule is calculated with the $\Delta_{-}$ and $\Delta_{+}$ backwards and forward diffs (respectively) for the $z$ grid (with $\Delta_{1,-} = 0$ and $\Delta_{I,+} = 0$ set for the ghost nodes),
\begin{align}
...
Let $v_1(t)$ be the $1$st element in the $v(t)$ vector, corresponding to $v(0,t)$.  Then, the value matching condition in 

	v_1(t) &= \omega \cdot v(t) - x(t)\label{eq:discretized-simple-vm}\\


The PDE in \cref{eq:simple-summary1} becomes the system of ODEs
\begin{align}
