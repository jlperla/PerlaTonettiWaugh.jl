...
\end{align}
To calculate the $\D[t]\log\left(1 - \tilde{L}(t)\right)$ we need to reference the ``future'' $\tilde{L}(t+\epsilon)$.  Since the algorithm solves the transition dynamics backwards, this can be done using the previous (i.e. future) timestep, and then forward first-differences for a given $\tilde{L}(t)$.  That is,
\begin{align}
	\D[t]\log\left(1 - \tilde{L}(t)\right) &\approx \frac{\log\left(1 - \tilde{L}(t+ \Delta)\right) - \log\left(1 - \tilde{L}(t)\right)}{\Delta}
\end{align}
where the $\Delta$ comes whatever the times-stepping algorithm.
