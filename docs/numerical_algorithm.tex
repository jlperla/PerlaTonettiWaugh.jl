...
\begin{align}
	\D[t]\tilde{v}(z,t) &= \underbrace{\left(\left(r - g(t)- \xi((\mu + \upsilon^2/2)-g(t)) - \frac{\upsilon^2}{2}\xi^2\right) - ((\mu + \upsilon^2/2) + \upsilon^2\xi - g(t)) \D[z] - \tfrac{\upsilon^2}{2} \D[zz]\right)}_{\equiv \tilde{A}(t)}\tilde{v}(z,t)  - \tilde{\pi}(z,t) \label{eq:bellman-GBM-dynamic-normalized-rearranged}\\
	\intertext{Then with this linear differential operator,}
	\D[t]\tilde{v}(z,t) &= \tilde{A}(t) \tilde{v}(z,t) - \tilde{\pi}(z,t)
\end{align}

Assume upwind finite difference discretization of the differential operators subject to the homogenous boundary values, such that \cref{eq:new-BC1,eq:new-BC2} hold, which creates $L^{-}_1$ and $L_2$ from \cref{sec:discretization-operators}.\footnote{Here, we will check that $((\mu + \upsilon^2/2) + \sigma^2\xi - g(t)) < 0$ so that $L^{-}_1$ is backwards differences, and consequently the correct upwind direction.}
...
%	\D[t]\tilde{v}(z,t) &= \left(\tilde{\rho}(t)  - (\mu - g(t) + (\sigma - 1)\upsilon^2)\D[z] - \frac{\upsilon^2}{2}\D[zz]\right)\tilde{v}(z,t) - \tilde{\pi}(z,t) \\	
	...
	\tilde{v}(0,t) &= \theta \int_{0}^{\infty}\tilde{v}(z,t) e^{(-\theta + \sigma - 1)z} \diff z - \zeta\label{eq:normalized-vm-summary-rescaled}\\
...
From (D.19) and using $\tilde{v}(t,0) = v(t,0)$, we see that if $E(t) > 0$ that,
\begin{align}
\tilde{v}(t,0) &= \zeta \frac{1-\chi}{\chi}
\end{align}

In the case that there is no entry, and $E(t) = 0$ for some $t$, from (D.18)
\begin{align}
\frac{\zeta}{\chi} &> \int_{0}^{\infty}v(z,t) f(z) \diff z\\
\intertext{Use $\tilde{v}(t,0) = v(t,0)$ and the adoption definition in \cref{eq:normalized-vm-summary-rescaled},}
\frac{\zeta}{\chi}&> \tilde{v}(0,t) + \zeta\\
\intertext{Reorganizing, this means that $E(t) = 0$ if}
\tilde{v}(0,t) &< \zeta \frac{1-\chi}{\chi}
\end{align}
...
\begin{align}
	0 &= \tilde{v}_1(t) - \omega \cdot \tilde{v}(t) + \zeta
\end{align}
...
\begin{align}
	\tilde{v}_1(t) &\leq \zeta \frac{1-\chi}{\chi},\, = \text{ if } E(t) > 0
\end{align}
...
Solving \cref{eq:bellman-GBM-dynamic} for $v(z,t)$ is problematic since the scale goes from approximatively $1$ to $e^{\bar{z}}$, which is very large for a high $\bar{z}$.  To make the solution more stable, we rescale the equation.  Take \cref{eq:sp-GBM-dynamic} and let $\tilde{v}(z,t) \equiv e^{-\xi z}v(z,t)$ for some $\xi \geq 0$ chosen for convenience and stability.  Then,
\begin{align}
...
% \intertext{There does not seem to be an obvious clean expression for $1-\tilde{L}$ in terms of the home share, but expression would be cleaner with it.  Note that (G.32) is not allowed since it is only at the stationary level. Using \cref{eq:L-tilde-sub} directly,}
% 1 - \tilde{L}(t) &= 1 - \Omega(t)\left((N -1)\hat{z}(t)^{-\theta}\kappa + \zeta \theta g(t) + \zeta \indicator{t \geq \bar{T}}\delta/\chi\right)\label{eq:minus-tilde-L}
% %\D[t]\log\left(1 - \tilde{L}(t)\right) &= \D[t]\log\left(1 - \Omega(t)\left((N -1)\hat{z}(t)^{-\theta}\kappa + \zeta \theta g(t)\right)\right)\\
% %&= \frac{\zeta  \theta  \left(g'(t)+g(t) g_{\Omega}(t)\right)+\kappa  (N-1) \hat{z}(t)^{-\theta } \left(g_{\Omega}(t)-\frac{\theta  \hat{z}'(t)}{\hat{z}(t)}\right)}{\zeta  \theta  g(t)+\kappa  (N-1) \hat{z}(t)^{-\theta }-\frac{1}{\Omega (t)}}\label{eq:D-t-tilde-L}
% %\intertext{Another formula is,}
% %&=\frac{\tilde{L}(t)\left(\theta g_{\hat{z}}(t)-g_{\Omega}(t)\right) - \zeta\theta\Omega(t)\left(\theta g(t) g_{\hat{z}}(t) + g'(t)\right)}{1-\tilde{L}(t)}\label{eq:D-t-tilde-L-alt}
 \end{align}
