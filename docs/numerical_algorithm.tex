...
Summarizing the discretized differential operators subject to the boundary conditions.  Let $L_1^{-}$ be the discretized backwards first differences and $L_2$ be the discretized central differences subject to the Neumann boundary conditions in \cref{eq:new-BC1,eq:new-BC2} such that $L_1^{-} v(t)$ and $L_2 v(t)$ represent the first and second derivatives of $v(z,t)$ respectively at $t$. For second derivatives, we use the following numerical scheme (\textbf{TODO: add reference to [Heterogeneous Agent Models in Continuous Time] by Achdou et al. (2017)}):

\begin{equation}
v''(z_i,t) \approx \dfrac{ \Delta_{i,-} v( z_i + \Delta_{i,+}, t) - (\Delta_{i,+} + \Delta_{i,-}) v( z_i , t) + \Delta_{i,+} v( z_i - \Delta_{i,-}, t)}{\frac{1}{2}(\Delta_{i,+} + \Delta_{i,-}) \Delta_{i,+} \Delta_{i,-} }, \text{for } i = 1, \ldots, P
\end{equation}
...
\begin{align}
&\dfrac{\Delta_{1,-} v( \Delta_{1,+}, t) - (\Delta_{1,+} + \Delta_{1,-}) v( 0 , t) + \Delta_{1,+}  v( - \Delta_{1,-}, t)}{\frac{1}{2}(\Delta_{1,+} + \Delta_{1,-}) \Delta_{1,+} \Delta_{1,-} } \\
&=
...
&= \dfrac{v(\Delta_{1, +}, t) - v(0, t)}{\Delta_{1, +}^2} + \dfrac{1}{\Delta_{i,+}} \xi v(0, t)  \\
&= \dfrac{1}{\Delta_{1, +}^2}  (- 1 + \Delta_{1, +} \xi) v(0,t)  + \dfrac{1}{\Delta_{1, +}^2}  v(\Delta_{1, +}, t)
\end{align}
Similarly, for \eqref{eq:new-BC2}, we have
\begin{align}
&\dfrac{\Delta_{P,-} v( \bar{z} + \Delta_{P,+}, t) - (\Delta_{P,+} + \Delta_{P,-}) v(\bar{z} , t) + \Delta_{P,+}  v( \bar{z} - \Delta_{P,-}, t)}{\frac{1}{2}(\Delta_{P,+} + \Delta_{P,-}) \Delta_{P,+} \Delta_{P,-} } \\
&=\dfrac{v (\bar{z} + \Delta_{P,-}, t) - 2 v(\bar{z} , t) + v(\bar{z} -\Delta_{P,-}, t)}{\Delta_{P,-}^2} \\
&=   \dfrac{v(\bar{z} - \Delta_{P,-}, t) - v(\bar{z}, t)}{\Delta_{P,-}^2} + \dfrac{1}{\Delta_{P,-}}\dfrac{ v(\bar{z}+\Delta_{P,-}, t) - v (\bar{z}, t) }{\Delta_{P,-}}  \\
...
From the technical appendix of Benhabib, Perla, and Tonetti (2019) (but renaming the $v$ function to be $\hat{v}$ here to avoid confusion with our normalization), assume an exogenously given $\hat{\pi}(z,t)$ and $x(t)$ function.  We will only look at examples where $F(z) = 1 - e^{-\theta z}$ for all $t$.
\begin{align}
...
\begin{align}
	...
	&= \theta\frac{1 - \hat{z}(t)^{-\theta + \sigma - 1}}{\theta - \sigma + 1} + (1 + (N-1)d^{1-\sigma})\theta\frac{\hat{z}(t)^{-\theta + \sigma - 1}}{\theta - \sigma + 1} + (N-1)\frac{\kappa}{\bar{\pi}_{\min}(t)}\theta \hat{z}(t)^{-\theta}
\end{align}
