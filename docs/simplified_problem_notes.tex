...
\begin{align}
	\left(r - \gamma - \frac{\sigma^2}{2}\xi^2\right) \tilde{v}(z,t) &= \tilde{\pi}(z,t) + (\gamma + \sigma^2\xi - g(t)) \D[z] \tilde{v}(z,t) + \tfrac{\sigma^2}{2} \D[zz] \tilde{v}(z,t) + \D[t]\tilde{v}(z,t)\label{eq:bellman-GBM-dynamic-normalized}
\end{align}
...
% v(0,t) &= \int_{0}^{\infty} v(z,t) F'(z)\diff z - x(t)\label{eq:vm-GBM-dynamic}\\
% \D[z]v(0,t) &= 0\label{eq:sp-GBM-dynamic-normalized}
% \end{align}
...
\begin{align}
\D[t]v(z_i, t_n) &\approx \frac{v_i^{n+1} - v_i^n}{h_{n,+}}\label{eq:D-v-t}\\
\D[z]v(z_i, t_n) &\approx \frac{v_i^n - v_{i-1}^n}{\Delta_{i,-}}\label{eq:D-v-z}\\
\intertext{And use non-uniform central differences for the 2nd derivative\footnotemark}
\D[zz]v(z_i, t_n) &\approx \frac{\Delta_{i,-}v^n_{i+1} - (\Delta_{i,-}+\Delta_{i,+})v^n_i+\Delta_{i,+}v^n_{i-1}}{\frac{1}{2}(\Delta_{i,+}+\Delta_{i,-})\Delta_{i,-}\Delta_{i,+}}\label{eq:D-v-zz}
\end{align}\footnotetext{If the $z$ grid is uniform, the central differences should end up as just
\begin{align}
	\D[zz]v(z_i, t_n) &\approx \frac{v_{i+1}^n - 2 v_i^n + v_{i-1}^n}{\Delta^2}\label{eq:D-v-zz}
\end{align}
}

Note that all of the time indices of the $\D[z]$ and $\D[zz]$ are at $n+1$, which I think is the key to making it implicit scheme in time.(Not sure here)
...
%	0 &= v_{2} - v_{1}\label{eq:sp-GBM-dynamic-discretized-stationary}\\
