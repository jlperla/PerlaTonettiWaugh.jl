@testset "solve_system" begin
    @testset "solve_system without nonlinear constraints" begin
        as = (a -> [a; (a+1)]).(0:10)
    ...
    @testset "solve_system with nonlinear constraints" begin
        ...
        for a in as
            h(x,a) = (x - a)
            x0 = [-8.0; -7.0; 0.0]
            @test solve_system(x -> h(x,a), x0; lb = fill(-20.0, length(x0)), constraints_fg! = constraints_fg!) ≈ a 
        end
    end
end