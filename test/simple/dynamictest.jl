...
# State grid.
    ...
    z_max = 5.0
    P = 100
    z_ex = range(z_min, stop = z_max, length = P) # Since we only care about the grid.
...
# Solve for the numerical stationary g_T.
    result_ns = stationary_numerical_simple(params_, z_ex)
...
# Create settings object.
    settings = @with_kw (z_ex = z_ex, T = T_val, g = t -> g_stationary,
                        ...
                        t_grid = range(0.0, stop = T_val, length = length(z_ex)))
